...
function tfidfCosine(queryText: string, song: Song, corpus: Song[]) {
  ...
  const terms = Array.from(new Set([...Object.keys(queryCounts), ...Object.keys(songCounts)]));
  if (!terms.length) {
    return 0;
  }
  ...
  for (const term of terms) {
    ...
    const queryWeight = (queryCounts[term] ?? 0) * idf;
    const songWeight = (songCounts[term] ?? 0) * idf;
